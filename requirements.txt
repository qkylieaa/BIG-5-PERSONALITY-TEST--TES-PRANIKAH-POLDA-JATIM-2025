...
if st.button("Proses Hasil"):
    ...
    output_file = "hasil_test_personality.xlsx"
    ...
    if not os.path.exists(output_file):
        df_baru.to_excel(output_file, index=False)
    else:
        # Jika file sudah ada → append
        df_lama = pd.read_excel(output_file)
        df_final = pd.concat([df_lama, df_baru], ignore_index=True)
        df_final.to_excel(output_file, index=False)

    st.success("Hasil berhasil disimpan ke Excel!")
